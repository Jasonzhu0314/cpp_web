...
每个新的客户端对应一个进程，创建一个新的进程处理客户端的连接，

父进程：关闭accept_fd， 非阻塞回收子进程

子进程：关闭listen_fd， 处理通信
...
循环的去判断当前文件描述符是否有状态返回，比如说判断调用是否阻塞

激活条件，简单来说

a）读缓冲区里有数据

b）**连接的读半部关闭**（也就是接收了FIN的TCP连接）。对这样的套接字的read将不阻塞并返回0（也就是返回EOF），select返回为1，主要用于通知服务器关闭连接，这部分跟管道关闭写端一样。

c）监听套接字有连接，该套接字是一个监听套接字且已完成的连接数不为0。对这样的套接字的 accept通常不会阻塞

d）其上有一个套接字错误待处理。对这样的套接字的读操作将不阻塞并返回-1（也就是返回一个错误），同时设errno。这些待处理错误（ pending error）也可以通过指定 SO ERROR套接字选项调用 get sockopt获取并清除。
